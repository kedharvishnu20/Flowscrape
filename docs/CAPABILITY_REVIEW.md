...
| 3   | ~~**MCP cannot scrape.**~~ **Closed by G-10**                                  | `pipeline_run` executes the pipeline's own emitted script — no second step engine, so what runs and what you export are one artefact                      |
